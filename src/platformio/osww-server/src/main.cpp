 ...
 * ledPin = set to GPIO8, the onboard addressable RGB LED on the ESP32-C6-DevKitC-1 (and pin-compatible boards). If you've wired a separate external LED instead, change this value to that GPIO pin.
...
// NOTE: pins updated for ESP32-C6-DEV-KIT-NX (RISC-V board).
// GPIO25/26/13 do not exist / are reserved (USB D-/D+) on the C6, so they were moved.
int directionalPinA = 18; // -> L298N IN1
int directionalPinB = 19; // -> L298N IN2
int ledPin = 8; // status LED GPIO. LED_BUILTIN is not usable here: this board
                  // has only an addressable RGB LED (GPIO8), so LED_BUILTIN
                  // resolves to an invalid pin (39) and crashes at boot.
                  // We drive that onboard RGB LED directly instead (no soldering
                  // needed) via neopixelWrite() - see utils/LedControl.cpp.
int externalButton = 20; // optional external ON/OFF button
...
void toggleDrawSavingIcon(bool showOnDisplay = false)
{
	if (showOnDisplay) 
	{
		...
		display.drawCircle(4, 4, 2, SH110X_WHITE);
		return;
	}
	
	// remove icon from screen buffer
	display.drawCircle(4, 4, 2, SH110X_BLACK);
	return;
}
...
static void drawStaticGUI(bool drawHeaderTitle = false, String title = "Winderoo") {
	if (OLED_ENABLED)
	{
		...
		// top horizontal line
		display.drawLine(0, 14, display.width(), 14, SH110X_WHITE);
		// vertical line
		display.drawLine(64, 14, 64, 50, SH110X_WHITE);
		// bottom horizontal line
		display.drawLine(0, 50, display.width(), 50, SH110X_WHITE);
...
static void drawTimerStatus() {
	if (OLED_ENABLED)
	{
		...
		{
			// right aligned timer
			display.fillRect(60, 54, 64, 13, SH110X_BLACK);
			display.setCursor(60, 56);
			display.print("TIMER " + userDefinedSettings.hour + ":" + userDefinedSettings.minutes);
		}
		else
		{
			display.fillRect(60, 54, 68, 13, SH110X_BLACK);
		}
	}
}
...
static void drawWifiStatus() {
	if (OLED_ENABLED)
	{
		...
		display.drawTriangle(4, 54, 10, 54, 7, 58, SH110X_WHITE);
		display.drawLine(7, 58, 7, 62, SH110X_WHITE);

		// Clear reception bars
		display.fillRect(12, 54, 58, 10, SH110X_BLACK);

		if (WiFi.RSSI() > -50)
		{
			// Excellent reception - 4 bars
			display.fillRect(14, 55+8, 2, 2, SH110X_WHITE);
			display.fillRect(18, 55+6, 2, 4, SH110X_WHITE);
			display.fillRect(22, 55+4, 2, 6, SH110X_WHITE);
			display.fillRect(26, 55+2, 2, 8, SH110X_WHITE);
...
static void drawProgressBar(float progress) {
	if (OLED_ENABLED && !screenSleep)
	{
		...
		display.fillRect(0, 50, display.width(), 2, SH110X_BLACK);
		
		// Calculate progress bar width
		int progressWidth = (int)(progress * display.width());
		
		// Draw the progress bar
		display.fillRect(0, 50, progressWidth, 2, SH110X_WHITE);
	}
}
...
static void drawDynamicGUI() {
	if (OLED_ENABLED && !screenSleep)
	{
		display.fillRect(8, 25, 54, 25, SH110X_BLACK);
		display.setCursor(8, 30);
		display.setTextSize(2);
		display.print(userDefinedSettings.rotationsPerDay);

		display.fillRect(66, 25, 62, 25, SH110X_BLACK);
...
static void drawNotification(String message) {
	if (OLED_ENABLED && !screenSleep)
	{
		...
		display.drawRect(0, 0, 128, 14, SH110X_WHITE);
		...
		delay(200);
		display.setCursor(0, 0);
		display.drawRect(0, 0, 128, 14, SH110X_BLACK);
		display.fillRect(0, 0, 128, 14, SH110X_BLACK);
		display.setTextColor(SH110X_WHITE);
		drawCentreStringToMemory(message.c_str(), 64, 3);
...
template <int N> static void drawMultiLineText(const String (&message)[N]) {
	if (OLED_ENABLED && !screenSleep)
	{
		...
		display.fillRect(0, 18, 128, 64, SH110X_BLACK);
...
void startWebserver()
{
	...
	server.on("/api/power", HTTP_POST, [](AsyncWebServerRequest *request) {
		// response is sent from the body handler below once the body is fully read
	...
	}).onBody([](AsyncWebServerRequest *request, uint8_t *data, size_t len, size_t index, size_t total)
	{
		...
		if (HOME_ASSISTANT_ENABLED)
		{
			...
			ha_rpd.setState(static_cast<int32_t>(requestTPD.toInt()));
			ha_selectDirection.setState(getDirectionIndexForHomeAssistant(requestRotationDirection));

			// Settings & Customization
			ha_customWindDuration.setState(static_cast<int32_t>(userDefinedSettings.customWindDuration.toInt()));
			ha_customWindPauseDuration.setState(static_cast<int32_t>(userDefinedSettings.customWindPauseDuration.toInt()));
			ha_customDurationInSecondsToCompleteOneRevolution.setState(static_cast<int32_t>(userDefinedSettings.customDurationInSecondsToCompleteOneRevolution));
...
void setup()
{
	...
	LED.begin(); // drives the onboard RGB LED (GPIO8) via neopixelWrite()

	// Quick boot self-test flash (white, ~300ms) so it's visible on every
	// single boot that the RGB LED wiring/code path works - independent of
	// whether this boot goes on to hit the WiFi-setup/reset/portal states.
	Serial.println("[STATUS] - LED boot self-test flash");
	LED.solidOn();
	delay(300);
	LED.off();
	...
	if(OLED_ENABLED)
	{
		Wire.begin(6, 7); // SDA=GPIO6, SCL=GPIO7 (already wired on the ESP32-C6 board)
	...
	if (wm.autoConnect("Winderoo Setup"))
	{
		...
		if (HOME_ASSISTANT_ENABLED) 
		{
			...
			ha_customDurationInSecondsToCompleteOneRevolution.setCurrentState(static_cast<int32_t>(userDefinedSettings.customDurationInSecondsToCompleteOneRevolution));
